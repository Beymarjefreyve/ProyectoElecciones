...
public class ResultadoService {
        ...
        public VotoResponse votar(Long eleccionId, VotoRequest req, String ip) {
                ...
                if (ahora.isBefore(eleccion.getFechaInicio())) {
                        throw new RuntimeException("La elección aún no ha comenzado. Fecha de inicio: "
                                        + eleccion.getFechaInicio());
                }
                if (ahora.isAfter(eleccion.getFechaFinaliza())) {
                        throw new RuntimeException("La elección ya ha finalizado. Fecha de finalización: "
                                        + eleccion.getFechaFinaliza());
                }

                Candidato candidato = candidatoRepo.findById(req.getCandidatoId())
                                .orElseThrow(() -> new RuntimeException("Candidato no existe"));

                // VALIDACIÓN NUEVA: Verificar que el candidato esté inscrito y ACTIVO en esta
                // elección
                inscripcionRepo.findByCandidatoIdAndEleccionIdAndEstado(req.getCandidatoId(), eleccionId, "ACTIVO")
                                .orElseThrow(() -> new RuntimeException(
                                                "El candidato no está inscrito o no ha sido aprobado para esta elección"));

                // Obtener el votante
                Votante votante = votanteRepo.findByDocumento(req.getDocumento())
                                .orElseThrow(() -> new RuntimeException("El documento " + req.getDocumento()
                                                + " no corresponde a un votante registrado"));

                // VALIDACIÓN NUEVA: Verificar estado del votante
                if (votante.getEstado() != null && !"ACTIVO".equalsIgnoreCase(votante.getEstado())) {
                        throw new RuntimeException("El votante no está activo. Estado actual: " + votante.getEstado());
                }

                // VALIDACIÓN 1: Verificar que el votante esté en el censo de esta elección
                Censo censo = censoRepo
                                .findByVotanteDocumentoAndEleccionId(req.getDocumento(), eleccionId)
                                .orElseThrow(() -> new RuntimeException(
                                                "El votante no está habilitado en el censo para votar en esta elección"));

                // VALIDACIÓN 2: Verificar restricción de facultad (si aplica)
                if (eleccion.getFacultad() != null) {
                        if (votante.getFacultad() == null
                                        || !votante.getFacultad().getId().equals(eleccion.getFacultad().getId())) {
                                throw new RuntimeException(
                                                "El votante no pertenece a la facultad requerida para esta elección: "
        ...
        public ParticipacionDTO calcularParticipacion(Long eleccionId) {
                ...
                double participacion = inscritos == 0 ? 0.0 : (votantes * 100.0) / inscritos;
        ...
        public void anularVoto(Long eleccionId, String documento) {
                ...
                Resultado resultado = repo.findByEleccionIdAndCensoId(eleccionId, censo.getId())
                                .orElseThrow(() -> new RuntimeException(
                                                "No se encontró un voto para este votante en esta elección"));
        ...
        private Censo registrarAutomaticamenteEnCenso(String documento, Eleccion eleccion) {
                Votante votante = votanteRepo.findByDocumento(documento)
                                .orElseThrow(() -> new RuntimeException("El documento " + documento
                                                + " no corresponde a un votante registrado"));

                // Si otro proceso ya lo registró mientras tanto, reutilizamos el registro
                // existente
        ...
        private List<ConteoPorFacultadDTO> calcularPorFacultad(List<Resultado> resultados) {
                ...
                return porFacultad.entrySet().stream()
                                .map(entry -> {
                                        ...
                                        String facultadNombre = entry.getValue().get(0).getEleccion().getFacultad()
                                                        .getNombre();
        ...
        private List<ConteoPorProgramaDTO> calcularPorPrograma(List<Resultado> resultados) {
                ...
                return porPrograma.entrySet().stream()
                                .map(entry -> {
                                        ...
                                        String programaNombre = entry.getValue().get(0).getEleccion().getPrograma()
                                                        .getNombre();
                                        List<Resultado> votos = entry.getValue();
